  ...
  return (
    <div className="min-h-screen bg-[var(--background)] text-[var(--foreground)] pb-20 transition-colors duration-300">
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 pt-6 md:pt-10">
        ...
        {/* ── Header (Optimized for Mobile Responsiveness) ── */}
        <motion.header
          initial={{ opacity: 0, y: -16 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5, ease: [0.23, 1, 0.32, 1] }}
          className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-8 md:mb-14 pb-5 border-b border-[var(--border)] w-full overflow-hidden"
        >
          {/* Logo Section */}
          <div className="flex items-center gap-3 md:gap-4 shrink-0 max-w-full">
            <div className="w-11 h-11 md:w-14 md:h-14 rounded-2xl bg-gradient-to-br from-purple-500 to-indigo-600 flex items-center justify-center shadow-lg shadow-purple-500/20 text-xl md:text-3xl select-none shrink-0">
              <motion.span 
            ...
            <div className="min-w-0">
              <h1
                className="font-display text-2xl sm:text-3xl md:text-4xl font-extrabold tracking-tight leading-none truncate"
                style={{
              ...
              <p className="text-[9px] md:text-xs font-bold tracking-[0.15em] sm:tracking-[0.2em] uppercase text-[var(--muted-foreground)] mt-1 truncate">
          ...
          <div className="flex items-center justify-between sm:justify-end gap-2 md:gap-3 w-full sm:w-auto shrink-0">
            <div className="flex items-center gap-2">
              ...
              <motion.button
                ...
                className="w-9 h-9 md:w-10 md:h-10 rounded-xl border border-[var(--border)] bg-[var(--muted)] flex items-center justify-center text-[var(--muted-foreground)] hover:text-[var(--foreground)] hover:bg-purple-500/10 hover:border-purple-500/30 transition-all cursor-pointer shrink-0"
              >
            ...
            {/* RainbowKit Connected Container Bounding Box */}
            <div className="max-w-[155px] sm:max-w-none overflow-hidden shrink-0">
        ...
        {!isConnected ? (
          <motion.div
            ...
            className="flex flex-col items-center justify-center min-h-[60vh] text-center max-w-xl mx-auto py-8 md:py-12 gap-6 md:gap-8"
          >
            <motion.div
              animate={{ y: [0, -12, 0] }}
            ...
            >
              ...
              <span className="relative text-7xl md:text-9xl select-none">🔐</span>
            </motion.div>

            <div className="space-y-3 md:space-y-4 px-2">
              <h2 className="font-display text-3xl sm:text-4xl md:text-6xl font-extrabold tracking-tight text-[var(--foreground)] leading-[1.1] sm:leading-[1.05]">
                Lock Today.
                <br />
                <span style={{
              ...
              <p className="text-xs md:text-base text-[var(--muted-foreground)] leading-relaxed max-w-sm mx-auto">
                A time-locked savings vault that protects your assets from impulse — engineered for disciplined wealth building.
              </p>
            </div>

            {/* How it Works Cards */}
            <div className="w-full max-w-sm glass-card lightning-edge rounded-2xl p-4 md:p-5 text-left space-y-3">
              <p className="text-xs font-bold uppercase tracking-widest text-[var(--muted-foreground)] mb-3 relative z-10">How it works</p>
              {[
                ...
                >
                  ...
                  <span className="text-xs sm:text-sm text-[var(--muted-foreground)] font-medium">{label}</span>
                </motion.div>
              ))}
            </div>
          ...
          >
            ...
            <motion.div
              ...
              animate={{ opacity: 1, x: 0 }}
              transition={{ duration: 0.5, delay: 0.1 }}
              className="lg:col-span-5 space-y-5 w-full order-2 lg:order-1"
            >
            ...
            <motion.div
              ...
              className="lg:col-span-7 w-full space-y-4 order-1 lg:order-2"
            >
              <VaultStatusBar address={address!} />

              <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
                <TabsList
                  className="w-full grid grid-cols-4 gap-1 p-1 mb-5 rounded-2xl h-auto"
                  ...
                  {TABS.map(({ value, label, Icon }, idx) => {
                    ...
                    return (
                      <TabsTrigger
                        key={value}
                        value={value}
                        className="relative flex flex-col sm:flex-row items-center justify-center gap-1 sm:gap-1.5 font-bold py-2.5 sm:py-3 px-1 sm:px-2 rounded-xl text-[10px] sm:text-xs cursor-pointer transition-all text-[var(--muted-foreground)] data-[state=active]:text-white data-[state=active]:shadow-lg overflow-hidden"
                      ...
                      >
                        ...
                        <span className={`relative z-10 w-3.5 h-3.5 sm:w-4 sm:h-4 rounded-md flex items-center justify-center text-[8px] sm:text-[9px] font-black shrink-0 ${isActive ? 'bg-white/20 text-white' : 'bg-purple-500/15 text-purple-500 dark:text-purple-400'}`}>
                          {idx + 1}
                        </span>
